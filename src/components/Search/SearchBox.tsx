...
const SearchBox: React.FC = () => {
    ...
    const [voiceKeyword, setVoiceKeyword] = useRecoilState(voiceSearchState);
    ...
    useEffect(() => {
        if(voiceKeyword.trim()) {
            setKeyword(voiceKeyword);
        }
    }, [voiceKeyword]);

    useEffect(() => {
        if(keyword.trim() && keyword === voiceKeyword) {
            navigate(`/item?keyword=${encodeURIComponent(keyword)}`);
            setVoiceKeyword('');
        }
    },[keyword, voiceKeyword, navigate, setVoiceKeyword]);
